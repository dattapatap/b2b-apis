...
export const addCategoriesToProduct = asyncHandler(async (req, res) => {
    ...
    const populatedProduct = await Product.findById(product._id).populate("media", "url type metadata").populate("subcategories", "name slug heading image ");
...
export const addProductDescription = asyncHandler( async( req, res) => {
    ...
    const product = Product.findById(product_id).populate('product_unit')
