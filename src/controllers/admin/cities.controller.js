...
export const createCity = asyncHandler(async (req, res) => {
    ...
    try {
        await citySchema.validateAsync(
            {name, description, state, operation: "create"},
            {abortEarly: false},
        );
        ...
        if (req.file.size > maxFileSize) {
            return res
                .status(400)
                .json(new ApiResponse(400, null, "File size exceeds the 1MB limit."));
        }
...
export const getAllCities = asyncHandler(async (req, res) => {
    ...
    const cities = await Cities.find({isDeleted: false})
        .skip(skip)
        .limit(limit)
...
export const updateCity = asyncHandler(async (req, res) => {
    ...
    try {
        await citySchema.validateAsync(
            {name, description, state, id, operation: "update"},
            {abortEarly: false},
        );
...
export const deleteCity = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!city) {
            return res
                .status(404)
                .json(new ApiResponse(404, null, "City not found or already deleted"));
        }
