...
export const createCountry = asyncHandler(async (req, res) => {
    ...
    let imageUrl;
    try {
        await CountrySchema.validateAsync(
            {country_name, country_code, operation: "create"},
            {abortEarly: false},
        );
        ...
        if (req.file.size > maxFileSize) {
            return res
                .status(400)
                .json(new ApiResponse(400, null, "File size exceeds the 2MB limit."));
        }
        ...
        return res
            .status(201)
            .json(new ApiResponse(201, country, "Countries created successfully"));
    } catch (error) {
        if (imageUrl) {
...
export const updateCountry = asyncHandler(async (req, res) => {
    const {id} = req.params;
    const {country_name, country_icon, country_code} = req.body;
    ...
    const updatedCountry = await Countries.findByIdAndUpdate(
        id,
        {country_name, country_icon, country_code},
        {new: true},
    );
